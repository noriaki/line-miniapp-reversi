...
- [x] 11.2 (P) AWS SDK依存関係を追加する
  - `pnpm add @aws-sdk/client-s3 @aws-sdk/s3-request-presigner`でパッケージを追加する
  - R2はS3互換APIを使用するためAWS SDKを利用する
  - _Requirements: 4.6_

- [x] 11.3 Presigned URL発行API Routeを実装する
  - `src/app/api/upload/presigned/route.ts`を新規作成する
  - `packages/mock-share-api/src/server.ts`のロジックを参考に実装する
  - 環境変数からR2認証情報を取得する
  - _Requirements: 4.6, 8.2_

- [x] 11.4 image-uploaderのデフォルトURLを更新する
  - 外部URL → 相対パス`/api/upload/presigned`に変更する
  - 環境変数未設定時のフォールバックを更新する
  - _Requirements: 4.6_

- [x] 11.5 (P) mock-share-apiパッケージを削除する
  - `packages/mock-share-api/`ディレクトリを削除する
  - `pnpm-workspace.yaml`から参照を削除する
  - `package.json`から`dev:mock`スクリプトを削除する
  - _Requirements: N/A（クリーンアップ）_

- [x] 11.6 (P) .env.exampleにR2環境変数テンプレートを追加する
  - R2_ENDPOINT, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET_NAME, R2_PUBLIC_URLを追加する
  - コメントで取得方法を説明する
  - _Requirements: 4.6_

- [x] 11.7 既存テストの期待値を更新する
  - `image-uploader.test.ts`のデフォルトURL期待値を更新する
  - 全テストがパスすることを確認する
  - _Requirements: 8.1_
...
| 4.4         | 2.1, 7.2                              |
| 4.5         | 2.1, 7.2                              |
| 4.6         | 2.3, 3.1, 5.1, 11.2, 11.3, 11.4, 11.6 |
| 5.1         | 4.1, 10.3                             |
| 5.2         | 4.1, 10.3                             |
| 5.3         | 4.1, 10.3                             |
| 5.4         | 4.1, 10.3                             |
...
| 7.3         | 8.1, 10.7                             |
| 7.4         | 8.1, 10.7                             |
| 8.1         | 2.2, 7.2, 10.5, 10.6, 11.1, 11.7      |
| 8.2         | 2.3, 3.1, 11.1, 11.3                  |
