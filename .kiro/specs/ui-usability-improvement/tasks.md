...
- [ ] 5.1 GameBoardコンポーネントのユニットテスト追加
  - 棋譜要素に `sr-only` クラスが適用されていることを検証するテスト
  - 棋譜要素に `aria-hidden="true"` 属性が設定されていることを検証するテスト
  - `data-testid="move-history"` 属性が保持されていることを検証するテスト
  - メッセージ表示領域に固定高さクラス（h-16）が適用されていることを検証するテスト
  - メッセージ要素が常にDOM内に存在することを検証するテスト
  - パス通知メッセージ表示時に `opacity-100` クラスが適用されることを検証するテスト
  - パス通知メッセージ非表示時に `opacity-0` クラスが適用されることを検証するテスト
  - `transition-opacity` クラスが適用されていることを検証するテスト
  - _Requirements: 4.3_

- [ ] 5.2 useGameErrorHandlerフックのユニットテスト追加
  - notifyPass関数が5秒後にpassNotificationをnullにクリアすることを検証するテスト
  - 新しいnotifyPass呼び出しが前のタイマーをキャンセルすることを検証するテスト
  - unmount時にタイマーがクリーンアップされることを検証するテスト
  - _Requirements: 3.1, 3.2, 3.3_
